...
public class AppointmentController {
	...
	public String saveApp(@Valid Appointment app, BindingResult result,@RequestParam(value = "action", required = true) String action) {
	...
	public String updateApp(@Valid Appointment app,BindingResult result, @PathVariable("id") long id,
			@RequestParam(value = "action", required = true) String action) {
		if (!action.equals("Cancel")) {
			
			if(result.hasErrors()) {
				return "redirect:/apps/edit/{id}";
			}
			appointmentService.save(app);
		}
		return "redirect:/apps/";
